...
type Attendance = { date: string; status: string; firstSeenAt?: string; faceImage?: string; course_code: string; title: string; room: string; period: number; startTime: string; endTime: string };
...
function AttendanceHistory() {
  ...
  const filteredRows = rows.filter((row) => matchesSearch(query, row.date, row.course_code, row.title, row.room, `Ca ${row.period}`, row.startTime, row.endTime, attendanceLabel[row.status as keyof typeof attendanceLabel] ?? row.status));
  return <section className="panel"><div className="panel-heading"><div><h2>Lịch sử điểm danh</h2></div><ListSearch id="attendance-search" value={query} onChange={setQuery} placeholder="Tìm môn, ngày, phòng, ca" /></div>{error ? <p className="notice error">{error}</p> : <table><thead><tr><th>Ngày</th><th>Môn học</th><th>Lớp / ca học</th><th>Ghi nhận</th><th>Trạng thái</th><th>Xác minh</th></tr></thead><tbody>{filteredRows.length ? filteredRows.map((row) => <tr key={`${row.date}-${row.course_code}-${row.period}-${row.startTime}`}><td>{row.date}</td><td><b>{row.course_code}</b><br /><small>{row.title}</small></td><td>{row.room}<br /><small>Ca {row.period} · {row.startTime} – {row.endTime}</small></td><td>{row.firstSeenAt ? new Date(row.firstSeenAt).toLocaleTimeString("vi-VN") : "—"}</td><td><span className={`status ${row.status}`}>{attendanceLabel[row.status as keyof typeof attendanceLabel] ?? row.status}</span></td><td>{row.faceImage ? <img className="attendance-proof" src={`data:image/jpeg;base64,${row.faceImage}`} alt={`Ảnh xác minh điểm danh ${row.course_code} ngày ${row.date}`} /> : "—"}</td></tr>) : <tr><td colSpan={6} className="empty">{rows.length ? "Không tìm thấy dữ liệu phù hợp." : "Chưa có dữ liệu điểm danh."}</td></tr>}</tbody></table>}</section>;
}
...
function AdminClasses() {
  const [courses, setCourses] = useState<Course[]>([]); const [users, setUsers] = useState<StoredUser[]>([]); const [sections, setSections] = useState<Section[]>([]); const [message, setMessage] = useState(""); const [query, setQuery] = useState(""); const [weekday, setWeekday] = useState(0); const [period, setPeriod] = useState(1); const [scheduleSlots, setScheduleSlots] = useState<Array<{ weekday: number; period: number }>>([]);
  ...
  const addScheduleSlot = () => setScheduleSlots((current) => current.some((slot) => slot.weekday === weekday && slot.period === period) ? current : [...current, { weekday, period }]);
  const createSchedule = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!scheduleSlots.length) return setMessage("Thêm ít nhất một ca học."); const form = new FormData(event.currentTarget); const courseCode = String(form.get("courseCode")); const teacherId = String(form.get("teacherId")); const room = String(form.get("room")); try { await Promise.all(scheduleSlots.map((slot) => api("/api/sections", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ courseCode, teacherId, room, ...slot }) }))); event.currentTarget.reset(); setScheduleSlots([]); await load(); setMessage(`Đã tạo ${scheduleSlots.length} ca học phần.`); } catch (cause) { setMessage(cause instanceof Error ? cause.message : "Không thể tạo lịch học."); } };
